...
@st.cache_resource(show_spinner="Loading models and precomputed reports…")
def load_all():
    reports = load_reports()
    have_reports = not reports["full_portfolio"].empty and not reports["scored_portfolio"].empty
    ...
    try:
        ...
        # Only this first-run path touches the raw source file, and only at
        # the training sample size — not the full multi-million-row CSV.
    ...
    df = reports["full_portfolio"]
    pd_sc = reports["scored_portfolio"]["pd_scorecard"].to_numpy()

    metrics = {}
    if config.METRICS_JSON.exists():
        metrics = json.loads(config.METRICS_JSON.read_text(encoding="utf-8"))
    return encoder, scorecard, gbm, rf, df, pd_sc, metrics, reports


try:
    encoder, scorecard, gbm, rf, df, pd_sc, metrics, reports = load_all()
except FileNotFoundError as exc:
    st.error(str(exc))
    st.stop()

# Static — doesn't require touching any data file.
spec = config.LENDING_CLUB_SPEC
DATASET_NAME = "Lending Club"

st.title("🏦 Credit Risk — PD Scorecard & IFRS 9 ECL")
st.caption(
    f"Data source: **{DATASET_NAME}** · {len(df):,} loans · "
    f"default rate {df[spec.target].mean():.1%}"
)
...
if page == "Overview":
    ...
    c2.metric("Default rate", f"{df[spec.target].mean():.1%}")
    c3.metric("Total EAD", f"${base_ecl['ead'].sum()/1e6:,.1f}M")
    c4.metric("Total ECL", f"${base_ecl['ecl'].sum()/1e6:,.1f}M")

    left, right = st.columns(2)
    with left:
        st.image(str(config.DASHBOARD_FIGURES_DIR / "score_distribution.png"), width="stretch")
    with right:
        st.image(str(config.DASHBOARD_FIGURES_DIR / "pd_by_purpose.png"), width="stretch")
...
if page == "Feature Analysis":
    ...
    st.image(str(config.DASHBOARD_FIGURES_DIR / "information_value.png"), width="stretch")

    feat = st.selectbox("Inspect a feature's WoE bins", list(encoder.bins_))
    binning = encoder.bins_[feat]
    woe_png = config.DASHBOARD_FIGURES_DIR / f"woe_{feat}.png"
    if woe_png.exists():
        st.image(str(woe_png), width="stretch")
    else:
        st.info("Chart not found for this feature — rerun `python scripts/train.py` to regenerate dashboard figures.")
    st.dataframe(binning.table, width="stretch")
...
if page == "Model Performance":
    ...
    with col1:
        st.image(str(config.DASHBOARD_FIGURES_DIR / "roc_curve.png"), width="stretch")
    with col2:
        st.image(str(config.DASHBOARD_FIGURES_DIR / "ks_curve.png"), width="stretch")
...
if page == "Validation":
    ...
    with col1:
        ...
        st.image(str(config.DASHBOARD_FIGURES_DIR / "calibration.png"), width="stretch")
    with col2:
        st.subheader("Rating masterscale")
        st.image(str(config.DASHBOARD_FIGURES_DIR / "masterscale.png"), width="stretch")
...
if page == "Score a borrower":
    ...
    with st.form("score_form"):
        cols = st.columns(3)
        inputs = {}
        for i, feat in enumerate(spec.numeric):
            col = cols[i % 3]
        ...
        for i, feat in enumerate(spec.categorical):
            col = cols[i % 3]
